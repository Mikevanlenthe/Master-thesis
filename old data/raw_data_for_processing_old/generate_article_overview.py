...
def generate_overview(read_path, remove_stopwords, use_lemmatizer, minimum_token_length, use_stemmer, use_bigrams, use_trigrams, args):
    ...
    for txt_file in txt_files:
        ...
        analysis_file = filter_name+"_analysis.txt"
        ...
        storage_directory_name = '/overview/'

        directory = str(filename[1]) + storage_directory_name

        if not os.path.exists(directory):
            os.makedirs(directory)

        # write article summaries
        with open(directory + analysis_file, "w") as outfile:
